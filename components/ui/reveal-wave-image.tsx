...
function ImagePlane({
  ...
  // Cover: always fill the full viewport regardless of image vs viewport aspect ratio
  const scale = useMemo<[number, number, number]>(() => {
    const vAR = viewport.width / viewport.height;
    if (aspectRatio > vAR) {
      // Image wider than viewport — fit to viewport height (cover)
      const sy = viewport.height / 2;
      return [sy * aspectRatio, sy, 1];
    } else {
      // Image taller — fit to viewport width (cover)
      const sx = viewport.width / 2;
      return [sx, sx / aspectRatio, 1];
    }
...
export const RevealWaveImage = ({
  ...
  const showCanvas = aspectRatio !== null && imageReady && !hasImageError;
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          backgroundImage: hasImageError ? undefined : `url(${src})`,
          filter: hasImageError ? undefined : 'grayscale(1) contrast(1.05)',
